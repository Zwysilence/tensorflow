...
class TensorBuffer;
class Tensor;
class EagerRecomputeHelper {
  ~EagerRecomputeHelper() = default;
  EagerRecomputeHelper(const EagerRecomputeHelper&) = delete;
  EagerRecomputeHelper(EagerRecomputeHelper&&) = delete;
  EagerRecomputeHelper& operator=(const EagerRecomputeHelper&) = delete;
  EagerRecomputeHelper& operator=(EagerRecomputeHelper&&) = delete;
 public:
  static EagerRecomputeHelper* GlobalEagerRecomputeHelper() {
    static EagerRecomputeHelper* helper = new EagerRecomputeHelper;
    return helper;
  }
  typedef std::function<void(const std::string&, const std::vector<std::string>&, const std::vector<std::string>&)> RecomputeCall;
  ...
  void SetRecomputing(const std::string& target);
  void SaveRecomputedTensor(const std::string& target, bool is_ref, const std::pair<std::string, Tensor*>& recomputed);
  int ShouldRefCount(const std::string& node_name) {
    if (node_input_ref_count_.count(node_name)) return node_input_ref_count_[node_name];
    return 0;
  }
 private:
  void SetRecomputedTensors(const std::string& target);
  EagerRecomputeHelper() { LoadRecomputePolicy(); }
  ...
  struct Params {
    ...
    bool then_delete;
    Params() : data_ready(DataStatus::OUT), buf(nullptr), using_count(0), then_delete(false) {}
  };
  ...
  struct TriggerInfo {
    ...
    TriggerInfo() : access_count(0), total_access_count(0), delete_trigger_count(0) {}
  };
  ...
  std::unordered_map<std::string, std::vector<std::string>> recompute_tensors_;
  std::unordered_map<std::string, int> node_input_ref_count_;
  std::mutex mu_;
};
}
